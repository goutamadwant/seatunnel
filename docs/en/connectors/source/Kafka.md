...
| strip_schema_registry_header        | Boolean                                                                    | No       | false                    | Effective when `format` is `avro` or `protobuf`. For Avro, `avro_schema` is required and the fixed 5-byte Confluent header is validated and stripped before decoding; the schema ID is not resolved. For Protobuf, the header and message indexes are detected and removed with fallback to plain Protobuf.                                                                                                                                                                                                                                                                    |
...
Use `format = avro` together with `avro_schema` when the Avro record layout (record name, namespace, or union structure) does not exactly match the SeaTunnel schema. When `avro_schema` is not provided, SeaTunnel derives the decode schema from the configured `schema` block and uses it as both reader and writer schema; set `avro_schema` explicitly whenever the producer's Avro layout (record name, namespace, union structure) differs from the SeaTunnel schema.

For messages produced by Confluent `KafkaAvroSerializer`, also set `strip_schema_registry_header = true`. This validates the magic byte and strips the fixed 5-byte header before decoding with `avro_schema`. SeaTunnel does not contact Schema Registry or resolve the schema ID. When stripping is enabled, plain Avro messages and malformed headers fail instead of falling back, because a valid plain Avro payload can also begin with zero.
...
`format = avro` reads plain Avro messages by default. For messages produced by Confluent `KafkaAvroSerializer`, configure both `avro_schema` and `strip_schema_registry_header = true`; SeaTunnel strips the 5-byte wire-format header and decodes with the configured writer schema without contacting Schema Registry.
